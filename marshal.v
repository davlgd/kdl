...
fn check_role(f FieldSpec, doc_level bool, has_args bool, has_props bool, path string) ! {
	...
	if !ok {
		return marshal_error(.unsupported, path, 'the `${tag_word(f.role)}` tag does not accept a ${kind_name(f.kind)} field')
	}
...
fn kind_name(k FieldKind) string {
	return match k {
		.scalar { 'scalar' }
		.list_scalar { 'array' }
		.list_struct { 'array of structs' }
		.map_scalar { 'map' }
		.struct_ { 'struct' }
		.opt_scalar, .opt_struct { 'option' }
	}
}
...
// encode_option writes the value of an option, and nothing for `none`.
...
fn (mut ctx MarshalContext) decode_option[E](_ ?E, v Value, idx int, children []Node, path string) !E {
	$if E is string || E is bool || E is $int || E is $float || E is $enum {
...
fn from_value[S](_ S, v Value, path string) !S {
	$if S is string {
		...
		$if S is f32 {
			if !math.is_nan(f) && !math.is_inf(f, 0) && math.abs(f) > math.max_f32 {
